...
async def upload_file_to_blob(file_name, container_name, blob_name=None):
...
async def download_file_from_blob(container_name, object_name, local_file_path):
...
async def download_and_send_file(bucket_name, object_name, peer_ip, peer_port):
    ...
    await download_file_from_blob(bucket_name, object_name, LOCAL_FILE_PATH_S3_FILE)
...
async def connect_with_reusable_port_dos(remote_host, remote_port, local_port, max_retries = 3):
...
async def interact_with_peer(reader, writer, data):
    ...
    while True:
        ...
        if processed_response == "DATA":
            ...
            upload_successful = upload_file_to_blob(LOCAL_FILE_RECEIVED_FILE, CONTAINER_NAME, new_file_name )
            upload_file_to_blob()
...
async def http_trigger(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        #await download_file_from_blob('data', '2mb.jpg', LOCAL_FILE_RECEIVED_FILE)
        #logger.info("done downloading")
        #await upload_file_to_blob(LOCAL_FILE_RECEIVED_FILE, 'data', generate_random_file_name('jpg'))
